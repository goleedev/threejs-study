...
const gltfs = [ '/ANIM_Griffon_Fly.glb', '/ANIM_Griffon_Fly.glb' ];
...
new GLTFLoader().load( gltfs[ selectModel ], function ( gltf ) {
  ...
  console.log(gltf);
  const morphAttributes = birdGeo && birdGeo.morphAttributes.position;
  const tHeight = nextPowerOf2( durationAnimation );
  const tWidth = birdGeo && nextPowerOf2( birdGeo.getAttribute( 'position' ).count );
  indicesPerBird = birdGeo && birdGeo.index.count;
  ...
  const totalVertices = birdGeo && birdGeo.getAttribute( 'position' ).count * 3 * BIRDS;
  ...
  let r = Math.random();
   for ( let i = 0; i < birdGeo && birdGeo.getAttribute( 'position' ).count * BIRDS; i ++ ) {
  ...
  for ( let i = 0; i < birdGeo && birdGeo.index.array.length * BIRDS; i ++ ) {
